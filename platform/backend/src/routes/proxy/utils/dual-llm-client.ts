...
export class MinimaxDualLlmClient implements DualLlmClient {
  ...
  async chatWithSchema<T>(
    ...
    // MiniMax supports JSON schema via response_format
    try {
      const response = await this.client.chat.completions.create({
        model: this.model,
        messages,
      ...
      logger.debug(
        ...
        "[dualLlmClient] MiniMax: structured output not supported or failed, using prompt fallback",
      );
...
Return only the JSON object, no other text.`;
      ...
      try {
        ...
        logger.error(
          { model: this.model, content: jsonText, parseError },
          "[dualLlmClient] MiniMax: failed to parse JSON response",
        );
        throw parseError;
      }
    }
